	...
	while (Global.ID == 1 && i < m){
		...
		while (change == 1){
			...
			for (s in setdiff(S.old, i)){
				if (graph.maxflow(graph.adjacency(L), source=s-1, target=i-1) == 0){
					S <- setdiff(S, s)
					change <- 1
					}
				}
			S.old <- S
			for (s in setdiff(S.old, i)){
				if (graph.maxflow(graph.adjacency(O, mode='undirected'), source=s-1, target=i-1) == 0){
					S <- setdiff(S, s)
					change <- 1
					}
				}
...
graphID.nonID <- function(L, O){
	...
	HTC.nonID <- (graph.maxflow( graph.adjacency(Cap.matrix), source=0, target=1) < sum(L) )	
...
graphID.genericID <- function(L, O, ILinv=0){
	...
	while (change == 1){
		...
		for (i in which(Solved.nodes == 0)){
			...
			flow <- graph.maxflow (graph.adjacency(Cap.matrix), source=0, target=1)
